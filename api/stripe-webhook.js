...
async function handleCheckoutCompleted(session) {
  ...
  // ---- 6. Auto-create deliveries row -------------------------------------
  // preference_token is auto-generated by the column default.
  const deliveryPayload = {
    order_id:       order.id,
    status:         'scheduled',
    auto_created:   true,
  }
  const { data: delivery, error: dlvErr } = await supabase
    .from('deliveries')
  ...
  // ---- 7. Send customer confirmation + preference link -------------------
  if (customerPayload.email && delivery?.preference_token) {
    try {
      await fetch(`${SITE_URL}/api/send-email`, {
        ...
        body: JSON.stringify({
          ...
          total,
          payment_type:    type,
          preferences_url: `${SITE_URL}/delivery-preferences.html?token=${encodeURIComponent(delivery.preference_token)}`,
        }),
      })
    } catch (err) {
