...
const { writeJson, research, supabaseAdmin, slugify, jobDeadline, researchDeadline } = require('./ai')
...
/*
 * generatePost({ topic, research, avoidTitles, plan, deadline }) — returns a
 * validated post object (not yet saved). `research` is optional
 * fact-gathering notes; `plan` is an optional blog_topic_queue row whose
 * brief, keywords, FAQ questions and internal links steer the article;
 * `deadline` (epoch ms) bounds the AI call.
 */
async function generatePost({ topic, research = '', avoidTitles = [], plan = null, deadline }) {
  ...
  return writeJson(prompt, BLOG_SCHEMA, { deadline })
}
...
async function freeChoiceTopic(avoidTitles, deadline) {
...
/*
 * writeAutoDraft({ onTopic, deadline }) — one run of the auto-writer. Takes
 * the highest-priority queued topic and writes it from its brief (still
 ...
 * `deadline` (epoch ms, see runBlogJob) splits the time between research and writing.
 */
async function writeAutoDraft({ onTopic, deadline } = {}) {
  ...
  if (planned) {
    ...
    const notes = await research(queueResearchPrompt(planned), { deadline: researchDeadline(deadline) })
    post = await generatePost({ topic: planned.working_title, plan: planned, research: notes, avoidTitles, deadline })
  } else {
    const { chosen, notes } = await freeChoiceTopic(avoidTitles, deadline)
    if (onTopic) await onTopic(chosen)
    post = await generatePost({ topic: chosen, research: notes, avoidTitles, deadline })
  }
...
/*
 * runBlogJob(jobId, work) — runs one blog_jobs row to completion and always
 * leaves it finished: 'done' with the post, or 'error' with the reason.
 * work({ deadline, setTopic }) must resolve to the saved post row.
 *
 * The routes run with maxDuration 300 (vercel.json). When Vercel kills an
 * invocation no catch block runs, which is how jobs used to sit 'running'
 * forever. So the work gets a deadline (JOB_BUDGET_MS, passed down to every
 * AI call), and a watchdog fails the job just after it, still inside the
 * platform limit. api/blog-job-status.js also expires anything left behind.
 *
 * Never throws. Resolves { post } or { error }.
 */
const WATCHDOG_GRACE_MS = 10000

async function runBlogJob(jobId, work) {
  const set = async (fields) => {
    try {
      const { error } = await supabaseAdmin.from('blog_jobs').update(fields).eq('id', jobId)
      if (error) console.error('[blog job] status update failed', jobId, error.message)
    } catch (err) {
      console.error('[blog job] status update failed', jobId, err)
    }
  }
  ...
  const watchdog = new Promise((_, reject) => {
    timer = setTimeout(
      () => reject(new Error('Timed out: the writer ran out of time. Please try again.')),
      deadline - Date.now() + WATCHDOG_GRACE_MS
    )
  })

  try {
    await set({ status: 'running' })
    const post = await Promise.race([work({ deadline, setTopic: (topic) => set({ topic }) }), watchdog])
    if (!post?.id) throw new Error('The writer finished without saving a post.')
    await set({ status: 'done', post_id: post.id, title: post.title, error: null })
    return { post }
  } catch (err) {
    console.error('[blog job] failed', jobId, err)
    const error = String(err?.message || err || 'Failed').slice(0, 500)
    await set({ status: 'error', error })
    return { error }
  } finally {
    clearTimeout(timer)
  }
