...
// Each Google call gets a deadline so a slow API can't eat the blog writer's time budget.
const FETCH_TIMEOUT_MS = 20000

// URL-prefix properties are registered with a trailing slash; without it the
// API answers 403 "User does not have sufficient permission" for every call.
function siteProperty() {
  const prop = (process.env.GSC_SITE_URL || '').trim()
  if (prop) return prop
