 ...
 *
 * Eric's signature is a PNG kept in private Supabase Storage, never in this
 * public repo. loadSignature(supabase) fetches it from SIGNATURE_STORAGE_PATH
 * (default contracts/brand/eric-signature.png); if it is missing, the
 * certificate sets his name in the script face instead.
 *
...
/* Eric's signature from private storage. Only a successful load is cached, so an upload is picked up without a redeploy. */
let signatureCache = null
async function loadSignature(supabase) {
  if (signatureCache) return signatureCache
  const [bucket, ...rest] = String(process.env.SIGNATURE_STORAGE_PATH || 'contracts/brand/eric-signature.png').split('/')
  try {
