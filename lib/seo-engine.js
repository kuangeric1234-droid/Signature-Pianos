...
const FRESH_GRACE_DAYS = 90 // a post with no recheck date is left alone this long after publishing
...
function decideNext({ posts, queue, runs }) {
  ...
  // Due for a recheck. A post with no recheck date (published by hand, or
  // before the engine existed) gets a grace period from its publish date,
  // so freshly published work isn't rechecked days after Eric posted it.
  const graceEnds = (x) => melbDate(new Date(new Date(x.published_at || x.created_at).getTime() + FRESH_GRACE_DAYS * 86400000))
  const due = posts
    .filter((x) => x.status === 'published' && !x.proposal && (x.reverify_by ? x.reverify_by <= today : graceEnds(x) <= today))
    .sort((a, b) => String(a.reverify_by || graceEnds(a)).localeCompare(String(b.reverify_by || graceEnds(b))))
...
const UPDATE_FIELDS = ['title', 'meta_description', 'excerpt', 'body_html', 'faq', 'tags', 'keywords']
