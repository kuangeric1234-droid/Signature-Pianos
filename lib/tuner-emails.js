 ...
 * sendEmail() wraps resend.emails.send so callers can tell a failed send.
 */
...
/*
 * Resend v3 never throws from emails.send(): a failed send comes back as
 * { data: null, error }. try/catch around it catches nothing, so every
 * caller used to count failed sends as sent. This returns null when the
 * email was accepted, or the error (never throws).
 *   const err = await sendEmail(resend, { from, to, subject, html })
 */
async function sendEmail(resend, message) {
  try {
    const result = await resend.emails.send(message)
    if (result && result.error) return result.error
    if (!result || !result.data) return { message: 'No response from Resend' }
    return null
  } catch (err) {
...
/* One line for logs and API responses. */
...
/* A small caps text link, e.g. "Open in Maps". `text` is plain text. */
function textLink(url, text, attrs = '') {
  return `<a href="${esc(url)}" ${attrs} style="font-family:${TEXT};font-size:11px;line-height:20px;letter-spacing:2px;text-transform:uppercase;color:${C.ink};">${esc(text)}</a>`
}
...
/* A row of small caps text links separated by dots. links: [url, text (plain), attrs?] */
function linkRow(links) {
  return `<p style="margin:12px 0 0;font-family:${TEXT};font-size:11px;line-height:22px;letter-spacing:2px;text-transform:uppercase;color:${C.inkSoft};">${
    links.map(([url, text, attrs]) => `<a href="${esc(url)}" ${attrs || ''} style="color:${C.ink};white-space:nowrap;">${esc(text)}</a>`).join(' &nbsp;·&nbsp; ')
  }</p>`
}
...
<body>
  ...
  <div class="rule"></div>
  <main class="card">
    ${lbl ? `<div class="label">${esc(lbl)}</div>` : ''}
    <h1>${esc(title)}</h1>
    ${body}
  </main>
  <div class="foot">
